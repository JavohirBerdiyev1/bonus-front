// DepositUsdTable.js (DatePicker o‘rniga MonthSelect bilan)
...
import DepositUsdService from "../services/deposit_usd_service";
import MonthSelect, { months } from "utils/MonthSelect";
import { getMonthRange } from "utils/getMonthRange";

const DepositUsdTable = () => {
  ...
  const fetchDeposits = async () => {
    try {
      ...
      const response = await DepositUsdService.getAll(pageIndex, pageSize, [start, end]);
 ...
 const columns = [
  ...
  {
    Header: "USD miqdori",
    accessor: "usd_deposit_amount", // to‘g‘rilandi
    Cell: ({ row }) => (
      <span className="ml-2">
        <NumberFormat
          value={row.original.usd_deposit_amount}
          displayType="text"
          thousandSeparator
          suffix=" USD"
        />
  ...
  {
    Header: "USD kursi",
    accessor: "usd_exchange_rate",
    Cell: ({ row }) => (
      <span className="ml-2">
        <NumberFormat
          value={row.original.usd_exchange_rate}
          displayType="text"
          thousandSeparator
          suffix=" UZS"
        />
...
export default DepositUsdTable;